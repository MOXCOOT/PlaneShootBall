...
//抽取向量点乘
inline double pvp_ai1_PointMul(double x0, double y0, double x1, double y1)
{
    return x0 * x1 + y0 * y1;
}
 ...
 * 找到最容易射中的球
 */
int pvp_ai1_FindBall(DataStruct *data){
    for(int i=data->ball_size - 1;i>=0;i--)
    {
        ...
        double kp=pvp_ai1_angleConver(data->plane1.angle);
 ...
 * 同时躲避两个球
 */
int pvp_ai1_Hideball(DataStruct *data, BallStruct &ball,KeyStruct*key)
{
    ...
    for(int i=0;i<data->ball_size;i++)
    {
        ...
        return pvp_ai1_vecMod(vx,vy);
    }

    for(int x=0;x<10;x++)
    {
        int a=pvp_ai1_FindBall(data);
        if(a)
        {
            if(data->ball[a].r<10)
            {
 ...
 * 返回值：0：瞄准成功，-1：瞄准失败
 */
int pvp_ai1_ShootBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{
    //获得两飞机间的距离
    float displanes = pvp_ai1_vecMod(data->plane1.x-data->plane2.x,data->plane1.y-data->plane2.y);
    ...
    if(displanes <= 500)
    {
        ball_x=data->plane2.x;
        ball_y=data->plane2.y;
        ball_v_x = data->plane2.v_x;
        ball_v_y = data->plane2.v_y;
    }
    ...
    double cosPosV;
    //两向量点乘除以两向量的模
    cosPosV = pvp_ai1_PointMul(ball_v_x, ball_v_y, data->plane1.x - ball_x, data->plane1.y - ball_y)
...
int pvp_ai1_HideBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{
 ...
 * 返回值：0：无需移动，1：需要移动
 */
int pvp_ai1_moveNeeded(DataStruct *data, float &des_x, float &des_y,KeyStruct*key, int preTime = 200)
{
    float min_t = INT_MAX;
    //预测每一个球的移动
    for (int i = 0; i < data->ball_size; i++)
    {
        ...
        float v=pvp_ai1_vecMod(ball_v_x,ball_v_y);
//        qDebug() <<v;
...
void pvp_ai1_move(time_t lastFetch,time_t fetch)
{
...
void ai_pvp_ai1(DataStruct*data,KeyStruct*key){
    ...
    //标志为0则正在移动或检查发现需要移动
    if (pvp_ai1_moveNeeded(data, des_x, des_y,key) || moveFlag == 0)
    {
        //进行移动并返回移动结果
    ...
    if (data->ball_size)
    {
        pvp_ai1_ShootBall(data, data->ball[pvp_ai1_FindBall(data)], key->rotate_left, key->rotate_right);
    }
