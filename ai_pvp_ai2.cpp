...
//抽取向量点乘
inline double pvp_ai2_PointMul(double x0, double y0, double x1, double y1)
{
    return x0 * x1 + y0 * y1;
}
...
int pvp_ai2_FindBall(DataStruct *data){
    for(int i=data->ball_size - 1;i>=0;i--)
    {
        //求得飞机与球之间的直线方程
        double px=data->plane2.x;
        double py=data->plane2.y;
        double bx=data->ball[i].x;
        double by=data->ball[i].y;
        double kb=(py-by)/(px-bx);
        double kp=pvp_ai2_angleConver(data->plane2.angle);
 ...
 * 同时躲避两个球
 */
int pvp_ai2_Hideball(DataStruct *data, BallStruct &ball,KeyStruct*key)
{
    ...
    for(int i=0;i<data->ball_size;i++)
    {
        ...
        return pvp_ai2_vecMod(vx,vy);
    }

    for(int x=0;x<10;x++)
    {
        int a=pvp_ai2_FindBall(data);
        if(a)
        {
            if(data->ball[a].r<10)
            {
 ...
 * 返回值：0：瞄准成功，-1：瞄准失败
 */
int pvp_ai2_ShootBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{

    //获得两飞机间的距离
    float displanes = pvp_ai2_vecMod(data->plane1.x-data->plane2.x,data->plane1.y-data->plane2.y);
    ...
    if(displanes <= 500)
    {
        ball_x=data->plane1.x;
        ball_y=data->plane1.y;
        ball_v_x = data->plane1.v_x;
        ball_v_y = data->plane1.v_y;
    }
    //瞄准的角度
    ...
    double cosPosV;
    //两向量点乘除以两向量的模
    cosPosV = pvp_ai2_PointMul(ball_v_x, ball_v_y, data->plane2.x - ball_x, data->plane2.y - ball_y)
    ...
    angleTo = pvp_ai2_angleConver(
                (asin(sqrt(1 - cosPosV * cosPosV) * pvp_ai2_vecMod(ball_v_x, ball_v_y) / 2000)
                 + atan2(ball_y - data->plane2.y, ball_x - data->plane2.x)) * 180 / Pai);
...
int pvp_ai2_HideBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{
    ...
    if(ball_v_x*ball_v_y>0)
    {
       ...
       if(ball_y>cross_y)
       {
           des_x+=data->ball->r+data->plane2.r;
       }
       else
       {
           des_x-=data->ball->r+data->plane2.r;
...
int pvp_ai2_turnTo(DataStruct *data, float x, float y, int &leftward, int &rightward)
{
    ...
    //计算飞机到目的地的角度并改变坐标系
    angleTo = pvp_ai2_angleConver(atan2(y - data->plane2.y, x - data->plane2.x) * 180 / Pai);
    //计算飞机朝向与该角度之差
    angleDiff = fmod(fmod(data->plane2.angle - angleTo, 360) + 360, 360);
 ...
 * 返回值：0：无需移动，1：需要移动
 */
int pvp_ai2_moveNeeded(DataStruct *data, float &des_x, float &des_y,KeyStruct*key, int preTime = 200)
{
    float min_t = INT_MAX;
    //预测每一个球的移动
    for (int i = 0; i < data->ball_size; i++)
    {
        ...
        float v=pvp_ai2_vecMod(ball_v_x,ball_v_y);
//        qDebug() <<v;
...
//        if(t < aftmin_t) aftmin_t = t;
        ...
        if (dis < data->plane2.r + ball.r + 10 && t < min_t)
        {
            ...
            if(ball_v_y > 0)
            {
                des_y = data->plane2.y -  ball.r * qAbs(data->plane2.y - cross_y)
            ...
            if(ball_v_x > 0)
            {
                des_x = data->plane2.x +  ball.r * qAbs(data->plane2.x - cross_x)
                        / pvp_ai2_vecMod(data->plane2.x - cross_x, data->plane2.y - cross_y);
                if(des_x > 1900)
                {
                    des_x = data->plane2.x -  ball.r * qAbs(data->plane2.x - cross_x)
            ...
            if(key->shield)
            {
                if(ball_v_x>0)
                {
                    des_x+=3 * data->ball->r+data->plane2.r;
                }
                else
                {
                    des_x-=3 * data->ball->r+data->plane2.r;
...
//            }
    ...
    for(int i=0;i<data->bullet1_size;i++)
    {
        ...
        //计算垂点坐标
        float cross_x = (data->plane2.y - bullet_y + bullet_v_y / bullet_v_x * bullet_x + bullet_v_x / bullet_v_y * data->plane2.x)
        ...
        //判断该子弹是否有威胁
        if (dis < data->plane2.r +  + 5 && t < min_t)
        {
            //设置最紧迫威胁
            min_t = t;
            if(bullet_v_y > 0)
            {
                des_y = data->plane2.y  - bullet.r * qAbs(data->plane2.y - cross_y)
                        / pvp_ai2_vecMod(data->plane2.x - cross_x, data->plane2.y - cross_y);
                if(des_y < 100)
                {
                    des_y = data->plane2.y   + bullet.r *qAbs(data->plane2.y - cross_y)
            ...
            if(bullet_v_x > 0)
            {
                des_x = data->plane2.x  + bullet.r *qAbs(data->plane2.x - cross_x)
                        / pvp_ai2_vecMod(data->plane2.x - cross_x, data->plane2.y - cross_y);
                if(des_x > 1900)
                {
                    des_x = data->plane2.x   - bullet.r *qAbs(data->plane2.x - cross_x)
...
void pvp_ai2_move(time_t lastFetch,time_t fetch)
{
...
void ai_pvp_ai2(DataStruct*data,KeyStruct*key){
    ...
    for(int i=0;i<data->ball_size;i++)
    {
        if(qAbs(data->ball->x - data->plane2.x) < 200 && qAbs(data->ball->y - data->plane2.y) < 300)
            cnt++;
    }
    if(cnt >= 3)
    {
    ...
    //标志为0则正在移动或检查发现需要移动
    if (pvp_ai2_moveNeeded(data, des_x, des_y,key) || moveFlag == 0)
    {
        //进行移动并返回移动结果
    ...
    if (data->ball_size)
    {
        pvp_ai2_ShootBall(data, data->ball[pvp_ai2_FindBall(data)], key->rotate_left, key->rotate_right);
    }
